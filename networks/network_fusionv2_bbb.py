...
class Decoder(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, num_layers: int, bayesian_final: bool = True):
        ...
        if self.bayesian_final:
            if self.num_layers > 0:
                self.final = BayesianConv2d(self.upconvs[-1].out_channels, self.num_classes*2, kernel_size=1, prior = torch.distributions.Normal(0, 1))
            else:
                self.final = BayesianConv2d(in_channels, self.num_classes*2, kernel_size=1, prior = torch.distributions.Normal(0, 1))
        else:
            if self.num_layers > 0:
    ...
    def forward(self, x):
        ...
        if x.shape != (x.shape[0], 3, 102, 180) and not self.warn_:
