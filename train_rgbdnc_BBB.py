...
from losses.weighted_gaussian_nll_loss import WeightedGaussianNLLLoss
from losses.lovasz_softmax import LovaszSoftmaxV1
from losses.activeboundaryloss import ABL

from config import configDict

from utils import Metrics, median_frequency_balancing, sigmoid_ramp_up
...
network_name = configDict['network']
...
def create_model(network_name, num_classes, dim_sources):
    if network_name == 'BayesianFusionNetworkV1':
        ...
        model = BayesianFusionNetworkV2(num_classes=num_classes, dim_sources=dim_sources)
...
dim_sources = [3, 1, 3, 1, 1]

model = create_model(network_name, num_classes=3, dim_sources=dim_sources)
...
model_summary = summary.summary(model, input_size=[x_rgb.shape, [x_depth.shape]])
...
sup_loss = WeightedGaussianNLLLoss(class_weights = weights, reduction='mean')
# iou loss
iou_loss = LovaszSoftmaxV1(reduction='mean')
# abl loss
abl_loss = ABL(reduction = 'mean', device = device)
...
for epoch in range(num_epochs):
    ...
    for phase in ['train', 'val']:
        ...
        for i, data in enumerate(dataloader[phase]):
            # this expt uses rgb, depth, normal, mean_curvature, gaussian_curvature
            rgb = data[0].to(device)
            depth = data[2].to(device)
            normal = data[5].to(device)
            mean_curvature = data[3].to(device)
            gaussian_curvature = data[4].to(device)
            
            # x3d contains the last k dim_sources. (RGB is not included)
            x3d = [depth, normal, mean_curvature, gaussian_curvature]
            
            assert len(x3d) == len(dim_sources) - 1
            
            for i in range(len(x3d)):
                assert x3d[i].shape[1] == dim_sources[i+1]
            ...
            output_mean, loss, sup_, \
            boundary_, complexity_ = model.sample_elbo_detailed_heteroscedastic_loss(
                inputs = [rgb, x3d],
                labels = labels_unhot,
                sample_nbr = 1,
                criteria = [sup_loss, iou_loss, abl_loss],
                complexity_cost_weight = 1/len(dataset['train'])
            )
            # metrics
            softmax = nn.Softmax(dim=1)
            y_pred = softmax(output_mean)
