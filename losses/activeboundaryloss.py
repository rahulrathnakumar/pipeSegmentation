...
from losses.label_smooth import LabelSmoothSoftmaxCEV1 as LSSCE
...
class ABL(nn.Module):
    def __init__(self, device, isdetach=True, max_N_ratio = 1/100, ignore_label = 255, label_smoothing=0.2, weight = None, max_clip_dist = 20., reduction='mean'):
    ...
    def forward(self, logits, target):
        ...
        dist_maps = self.get_dist_maps(gt_boundary).to(self.device) # <-- it will slow down the training, you can put it to dataloader.
        ...
        if self.reduction == 'mean':
            ...
            loss = (loss * weight_ce).sum()  # add distance weight
